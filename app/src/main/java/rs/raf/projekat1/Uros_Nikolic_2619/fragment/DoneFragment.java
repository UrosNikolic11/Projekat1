...
import rs.raf.projekat1.Uros_Nikolic_2619.recycler.adapter.TicketAdapter3;
...
public class DoneFragment extends Fragment {
    ...
    private TicketAdapter3 ticketAdapter;
    ...
    private void initListeners(){
        filter.addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                recyclerViewModel.filterTicketDone(s.toString());
            }
        });
    }

    private void initObservers(){
        recyclerViewModel.getDoneFilter().observe(this.getViewLifecycleOwner(), tickets -> {
    ...
    private void initRecycler() {
        ticketAdapter = new TicketAdapter3(new TicketDiffItemCallBack(), ticket -> {
                Intent intent = new Intent(this.getContext(), Prikaz2Activity.class);
                intent.putExtra(Prikaz2Activity.PRIKAZ2_KEY, ticket);
                startActivity(intent);

        }, ticket -> {
            recyclerViewModel.addInProgres(ticket.getId());
            filter.setText("");
        }, ticket -> {
            recyclerViewModel.removeToDo(ticket.getId());
            filter.setText("");
        }, user);
